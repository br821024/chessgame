...
public class MySQL {
	...
	private void createTable(){
		try{
			statement.executeUpdate("CREATE TABLE IF NOT EXISTS login(account_id INT(20) UNSIGNED NOT NULL AUTO_INCREMENT,user_id VARCHAR(32) NOT NULL,user_pass VARCHAR(32) NOT NULL ,user_name VARCHAR(32) NOT NULL DEFAULT \"Anonymous\", PRIMARY KEY(account_id) , UNIQUE(user_id));");
		}catch(SQLException e){}
	}
	...
	public String getUserName(String account){
		try{
			PreparedStatement prstmt = null;
			prstmt = connection.prepareStatement("SELECT user_name FROM login WHERE user_id = ?");
			prstmt.setString(1,account);
			resultset = prstmt.executeQuery();
